...
- [~] **Fase 6**: Tailscale / redes remotas y modo DJ — unirse al coordinador por IP en vez de mDNS (`start_joining`/`connect_to_coordinator`); modo DJ con **múltiples transmisores simultáneos**: `MediaConfig.dj` activa el mezclador (`DjMixer`, una fuente por transmisor compartiendo el reloj de sesión) y la sesión permite a varios dispositivos transmitir a la vez (`dj_transmit`). Pendiente de pulir en la app Flutter
